...
function NumberInput({
  ...
  // Only sync from parent when value changes, not when raw changes
  useEffect(() => { 
    const nv = value === "" ? "" : String(value); 
    setRaw(nv);
  }, [value]); // Remove raw from dependencies to prevent loops

  const commit = () => {
    const s = raw.trim();
    console.log("NumberInput commit:", { raw: s, value, placeholder }); // Debug log
    if (s === "") { 
      onCommit(""); 
      return; 
    }
    const n = Number(s);
    if (Number.isNaN(n)) { 
      console.log("Invalid number:", s); // Debug log
      onCommit(""); 
      setRaw(""); // Reset display if invalid
      return; 
    }
    const clamped = clamp(n, min, max);
    console.log("Committing value:", { original: n, clamped, min, max }); // Debug log
    onCommit(clamped);
    if (clamped !== n) {
      setRaw(String(clamped)); // Only update display if value was clamped
    }
  };
  ...
  return (
    <input
      ...
      onChange={(e) => {
        ...
        // Allow empty string, numbers, and decimal point (no negative for most use cases)
        // But be more permissive to allow typing
