  ...
  return (
    <div className="space-y-2">
      {items.map((item, index) => (
        ...
        >
          <div className="cursor-move text-zinc-200">⋮⋮</div>
          <Input
            value={item}
...
export default function TimeCalendarForm({ data, onUpdate }: TimeCalendarFormProps) {
  const addMonth = () => {
    const newMonths = [...data.months, { name: "", days: null }];
    onUpdate({ months: newMonths });
  };

  const removeMonth = (index: number) => {
    const newMonths = data.months.filter((_, i) => i !== index);
    onUpdate({ months: newMonths });
  };

  const updateMonth = (index: number, updates: Partial<MonthData>) => {
    const newMonths = data.months.map((month, i) => 
  ...
  const addWeekday = () => {
    if (data.weekdays.length < 14) {
      const newWeekdays = [...data.weekdays, ""];
  ...
  const removeWeekday = (index: number) => {
    if (data.weekdays.length > 2) {
      const newWeekdays = data.weekdays.filter((_, i) => i !== index);
      onUpdate({ weekdays: newWeekdays });
    }
  };

  const updateWeekday = (index: number, value: string) => {
    const newWeekdays = data.weekdays.map((day, i) => i === index ? value : day);
    onUpdate({ weekdays: newWeekdays });
  };
      ...
      <div>
        <div className="flex justify-between items-center mb-2">
          ...
          <button
            type="button"
            onClick={addWeekday}
            disabled={data.weekdays.length >= 14}
